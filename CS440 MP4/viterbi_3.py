...
def viterbi_3(train, test):
    ...
    for word in word_total:
        ...
        if word_total[word]==1:
                hapax_word.append(word)
                # print(word)
    ...
    for tag in hapax:
        hapax[tag]=hapax[tag]/len(hapax_word)
        #print(tag, hapax[tag])
    ...
    #print(hapax_suffix)  
    for sfx in hapax_suffix:
        hapax_suffix[sfx]=hapax_suffix[sfx]/suffix_count
    ...
    for sentence in test:
        ...
        for i in range(1,len(sentence)):
                ...
                for tagB in tag_total:
                        ...
                        max_v_prev_tag=None
                        possible_tag=None
                        possible_tag_PE=None
                        if word not in tag_word[tagB]:
                                possible_tag=suffix(word)
                        for tagA in tag_total:
                                ...
                                if hapax[tagB]!=0:
                                        laplace=0.0001
                                        laplace=laplace*hapax[tagB]
                                if word not in tag_word[tagB]:
                                        if possible_tag is not None:
                                                laplace=laplace*hapax_suffix[possible_tag]
                        ...
                        #                 laplace=0.0001
                        #                 laplace=laplace*hapax[tagB]
                        #         v[i][tagB]=possible_tag_v
                        #         b[i][tagB]=possible_tag
                                #print(word, possible_tag)
                        # print(v[i][tagB])
                        # print(b[i][tagB])
...
def suffix(word):
    if word.endswith("al") or word.endswith("ive") or word.endswith("ous") or word.endswith("ful"):
        return "ADJ"
    elif word.endswith("ment") or word.endswith("tion") or word.endswith("ity") or word.endswith("ism") or word.endswith("er") or word.endswith("ness") or word.endswith("age") or word.endswith("s"):
        return "NOUN"
